...
namespace ET
{
	...
	public class BuildEditor : EditorWindow
	{
        ...
        private void OnGUI() 
		{
			...
			if (GUILayout.Button("HybridCLRHotUpdateDllBuild"))
			{
				var rootWolongDir = SettingsUtil.GetHotUpdateDllsOutputDirByTarget(EditorUserBuildSettings.activeBuildTarget);
				Directory.CreateDirectory(rootWolongDir);

				BuildAssembliesHelper.BuildModel(this.codeOptimization, globalConfig, rootWolongDir);
				BuildAssembliesHelper.BuildHotfix(this.codeOptimization, globalConfig, rootWolongDir);

				ShowNotification("Build Model And Hotfix Success!");
			}

			if (GUILayout.Button("HybridCLRGenerateAll"))
			{
				// 顺序随意
				ReversePInvokeWrapperGeneratorCommand.GenerateReversePInvokeWrapper(false);
				...
				ShowNotification("HybridCLRGenerateAll Success!");
			}

			if (GUILayout.Button("CopyHybridCLRAssembliesToStreamingAssets"))
			{
